...
// Integração agrupada (Triagem isolada, sistemas com flag de IA, Outro com textarea)
const INTEGRACOES_GRUPOS: { grupo: string; itens: string[] }[] = [
  ...
  },
  {
    ...
    itens: [
      "Feegow",
      "Amigo Tech (Amigo App)",
      "Dr e Cia",
      "Sigma Computel",
      "ViSaúde",
      "ANImati / NetRIS",
      "Vizi Saúde / App Health",
      "Tecnarte / Visual Asa",
      "Quark Clinic",
      "ProntMed",
      "Klingo",
...
// Integrações que NÃO suportam checkbox "+ IA" (já são variantes da Triagem)
...
function NovoCliente() {
  ...
  // Monta string final da integração
  ...
  return (
    <div className="px-6 md:px-8 py-8 max-w-3xl mx-auto">
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        Dados básicos da empresa. Detalhes contratuais serão validados durante o kickoff.
      </p>
      ...
      <Card className="mt-6 p-6 border-border">
        <form onSubmit={submit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <FieldText
            ...
            type="number"
            value={f.idclinic}
            onChange={set("idclinic")}
            placeholder="ID numérico"
          />
          <FieldText
            label="Categoria"
            placeholder="Ex.: Odontologia, Médico"
          ...
          <FieldDropdown
            label="Vendedor"
            value={f.vendedor}
            onChange={(v) => setF({ ...f, vendedor: v })}
            opcoes={VENDEDORES}
          />
          <FieldDropdown
            label="Gerente de contas"
            value={f.gerente}
            onChange={(v) => setF({ ...f, gerente: v })}
            opcoes={GERENTES}
          />
          <FieldDropdown
            label="Configurador"
            value={f.configurador}
            onChange={(v) => setF({ ...f, configurador: v })}
            opcoes={CONFIGURADORES}
          />
          ...
          {/* Integração — dropdown agrupado + checkbox IA + textarea Outro */}
          <div className="md:col-span-2 space-y-1.5">
            ...
            {podeMarcarIa && (
              <label className="flex items-center gap-2 mt-2 cursor-pointer">
                <Checkbox
                  checked={f.integracao_com_ia}
                  onCheckedChange={(v) => setF({ ...f, integracao_com_ia: !!v })}
                />
            ...
            {f.integracao_base === "Outro" && (
              <Input
                value={f.integracao_outro}
                onChange={set("integracao_outro")}
                placeholder="Qual integração?"
                className="mt-2"
              />
            )}
